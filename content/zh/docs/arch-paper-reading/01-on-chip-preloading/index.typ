...
#let paper = "https://doi.org/10.1145/125826.125932"
...
#example(title: "用一条规则 load 走一遍")[
  ...
  那 `RPT` 大致会这样变化：
  1. 第一次看到 `1000`, `prev-addr = 1000`, `state = initial`
  2. 第二次看到 `1064`, 学到 `stride = 64`, `prev-addr = 1064`, `state = transient`
...
#figure(
  ...
  caption: [`RPT` entry 由 `PC tag / prev-addr / stride / state` 组成。`LA-PC` 是否能用一条记录发 preload，不只取决于有没有 stride，还取决于状态机是否已经进入 `steady`。],
)
...
这篇 paper 不从当前数据块继续外推，而是沿未来可能执行到的 load/store 指令提前查地址历史。因此它不是 “address-stream-centered”，而是更接近 “instruction-stream-centered” 的预取设计。
...
如果 RPT 预测“这条指令下一次会访问 `prev_addr + stride`”, 那系统就会检查这个 block：
- 在不在 cache
- 在不在 ORL 里已经请求过
...
#figure(
  ...
  caption: [`On-Chip Preloading` 的主路径是：分支预测推动 `LA-PC` 提前遍历控制流；当它命中已经训练过的访存 `PC` 时，读取 `RPT` 生成候选地址，再经过 `ORL / cache` 过滤后决定是否发 preload。],
)

==== Look-Ahead Distance：LA-PC 到底应该领先多远

理想情况是：`LA-PC` 领先 `PC` 的距离，刚好等于下一层 memory hierarchy 的访问延迟 `b`, 这样 preload 发出去后，能在真实 load 需要前把数据送到 cache。
...
同时还要考虑和 write buffer 的一致性问题：preload 和 demand miss 都要检查 write buffer，避免读到过期值或和未写回的数据冲突。
...
发现分支预测错误之前已经发出的 preload 回来以后，怎么处理呢? 有几种自然做法：
+ 让它正常填入 cache
  - 简单, 但可能造成 cache pollution, 这也是 branch misprediction 的代价之一
+ 给这类请求打“discard / ignore”标记
  - 数据回来时不装入 cache，直接丢掉。论文在别的场景里用了类似机制：例如 write miss 与 preload 冲突时，会给 ORL 项加 discard status，表示回来后忽略；同样的机制理论上也可以用于 branch flush 后的无效 preload
...
#figure(
  ...
  caption: [`On-Chip Preloading` 把训练流和发射流分开：真实执行流在地址计算后更新 `RPT`，`LA-PC` 只读取已经稳定的历史记录，并尝试发出 speculative preload。],
)
...
1. non-overlapped: 最保守的模型。一次 miss 基本会把后续推进卡死，等这次访存回来再说。这样的系统里，即使会预取，能发挥的空间也有限。
  ...
  ```
2. overlapped: 有一定重叠能力。一次 miss 期间，处理器还能做别的独立工作，所以 preload 如果能把数据提早放进 cache，就有机会缩短 demand path 上的等待时间。
  ```
  A: [issue][----latency----][transfer]
  B:        [issue][----latency----][transfer]
  ```
